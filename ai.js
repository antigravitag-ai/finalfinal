...
user_name = "Learner"</code></pre>
        ...
        <ul>
         ...
         <li><code>user_name = "Learner"</code>: Python allocates memory for the text sequence <code>"Learner"</code> and binds the name <code>user_name</code> to it.</li>
        </ul>`,
        example: `xp = 50
...
print(student["name"]) # Outputs "Learner"</code></pre>`,
        example: `profile = {"username": "learner", "xp": 100}
...
print(my_dog.speak()) # Outputs "Woof!"`,
  ...
  splitQuestions(prompt) {
    const normalized = String(prompt || '').replace(/\r\n/g, ' ').replace(/\s+/g, ' ').trim();
    if (!normalized) return [];

    const questionWords = '(what|why|how|when|where|which|who|can|could|should|do|does|is|are|explain|compare|describe|identify|tell|show|write|fix|why does|what does)';

    const byQuestionMark = normalized.split('?').map(part => part.trim()).filter(Boolean);
    if (byQuestionMark.length > 1) {
      return byQuestionMark.filter(part => part.length > 2).map(part => part.replace(/^[^A-Za-z]+/, '').replace(/[\s,;:]+$/, ''));
    }

    const withAndSplit = normalized.split(new RegExp(`\\s+and\\s+(?=${questionWords}\\b)`, 'i'));
    if (withAndSplit.length > 1) {
      return withAndSplit.filter(part => part.length > 2).map(part => part.replace(/^[^A-Za-z]+/, '').replace(/[\s,;:]+$/, ''));
    }

    const commaSplit = normalized.split(new RegExp(`\\s*,\\s*(?=${questionWords}\\b)`, 'i'));
    if (commaSplit.length > 1) {
      return commaSplit.filter(part => part.length > 2).map(part => part.replace(/^[^A-Za-z]+/, '').replace(/[\s,;:]+$/, ''));
    }

    return [normalized.replace(/[\s,;:]+$/, '')];
  }

  buildMultiQuestionResponse(questions, userState) {
    const items = questions.filter(Boolean).map(q => q.replace(/[?!.]+$/, '').trim()).filter(q => q.length > 2);
    if (items.length <= 1) return this.generateSingleAnswer(items[0] || '', userState, false);

    const sections = items.map((question, index) => {
      const answer = this.generateSingleAnswer(question, userState, true);
      return `<div style="margin-bottom: 20px; padding-bottom: 12px; border-bottom: 1px solid var(--border-color);"><h4 style="margin: 0 0 8px;">${index + 1}. ${question.replace(/^\w/, c => c.toUpperCase())}</h4>${answer}</div>`;
    }).join('');

    return `<h3>Python Q&A Breakdown</h3>${sections}`;
  }

  generateSingleAnswer(userPrompt, userState, compact = false) {
    const prompt = String(userPrompt || '').toLowerCase().trim();
    const userLevel = Number(userState?.profile?.level || 1);
    const modifier = userLevel > 3 ? 'intermediate' : 'beginner';

    if (!prompt) {
      return '<p>Ask me a Python question and I will explain it clearly.</p>';
    }

    if (this.isGreeting(prompt)) {
      return `<h3>Hello! 🤖</h3><p>I am your Python AI Teacher. I can explain Python concepts, review code, and help you debug errors in plain language.</p><p>Ask me about loops, variables, strings, functions, or an error message and I will break it down.</p>`;
    }

    if (this.lastTopic) {
      const activeTopic = this.topics[this.lastTopic];
      if (prompt.includes('example') || prompt.includes('code')) {
        return `<h4>Code Example for ${activeTopic.title}:</h4><pre><code class="python-code">${activeTopic.example}</code></pre><p>Try this in the workspace and compare the output step by step.</p>`;
      }
      if (prompt.includes('beginner') || prompt.includes('simple') || prompt.includes('analogy') || prompt.includes('explain like')) {
        return `<h4>Simple Analogy for ${activeTopic.title}:</h4><p>${activeTopic.beginner}</p><p>Does that help make the idea clearer?</p>`;
      }
      if (prompt.includes('hint') || prompt.includes('tip')) {
        return `<h4>Teacher Tip for ${activeTopic.title}:</h4><p>💡 ${activeTopic.hint}</p>`;
      }
    }

    if (/(runtime|syntax|logical)\s+error/.test(prompt) || /what\s+is\s+a\s+(runtime|syntax|logical)\s+error/.test(prompt) || /why\s+does\s+this\s+code\s+give\s+an\s+error/.test(prompt)) {
      const errorType = prompt.includes('runtime') ? 'runtime' : prompt.includes('logical') ? 'logical' : 'syntax';
      const exampleCode = errorType === 'runtime'
        ? 'num = 10\nprint(num / 0)'
        : errorType === 'logical'
        ? 'total = 0\nfor i in range(1, 5):\n    total += 1\nprint(total)'
        : 'if score > 50\n    print("Pass")';
      ...
      const definitions = {
        syntax: {
          title: 'Syntax Error',
          explanation: 'A syntax error happens when Python cannot read your code because the structure is invalid. Common causes are missing colons, mismatched parentheses, or broken indentation.',
          fix: 'Check the exact line Python points to and make sure your statements are properly formed.'
        },
        runtime: {
          title: 'Runtime Error',
          explanation: 'A runtime error occurs while the program is running. The code is valid, but Python hits a problem when it tries to execute it, such as dividing by zero or using a missing value.',
          fix: 'Validate inputs and guard against invalid operations before they happen.'
        },
        logical: {
          title: 'Logical Error',
          explanation: 'A logical error is when the program runs without crashing but produces the wrong result because the algorithm is incorrect.',
          fix: 'Trace the code step by step and test the variable values as the program runs.'
        }
      };

      const current = definitions[errorType];
      return `<h4>${current.title}</h4><p>${current.explanation}</p><pre><code class="python-code">${exampleCode}</code></pre><p><strong>How to fix it:</strong> ${current.fix}</p>`;
    }
    ...
    for (const [topicKey, topicData] of Object.entries(this.topics)) {
      ...
      topicData.keywords.forEach(kw => {
        ...
        if (regex.test(prompt)) matches += 1;
      });
    ...
    if (bestTopic) {
      ...
      if (compact) {
        return `<p>${this.topics[bestTopic].explanation.replace(/<[^>]+>/g, '')}</p>`;
      }
      return this.formatTopicResponse(this.topics[bestTopic], modifier);
    }

    if (prompt.includes('example') || prompt.includes('sample') || prompt.includes('show me')) {
      return `Here is a basic Python example:<pre><code class="python-code">name = "Pythonist"\nscore = 10 + 20\nprint(f"Hello {name}, your score is {score}")</code></pre><p>Ask me about variables, f-strings, or operators and I will explain each part.</p>`;
    }

    if (prompt.includes('hint')) {
      return `💡 Break the problem into small steps: identify the inputs, decide the logic, then test the output with a few sample values.`;
    }

    if (prompt.includes('quiz me')) {
      return `Sure — here is a quick Python check:<pre><code>x = 5\nx = x + 2\nprint(x)</code></pre><p>The answer is 7 because the value of x is updated before printing it.</p>`;
    }

    if (prompt.includes('variable') || prompt.includes('variables')) {
      return `<h3>Variables</h3><p>Variables store values so you can reuse them later. For example, <code>score = 10</code> stores 10, and then <code>print(score)</code> prints it.</p><pre><code class="python-code">score = 10\nscore = score + 5\nprint(score)</code></pre><p>The output is 15.</p>`;
    }

    if (prompt.includes('loop') || prompt.includes('for') || prompt.includes('while')) {
      return `<h3>Loops</h3><p>Loops repeat code without rewriting it. A <code>for</code> loop is useful when you know how many times to repeat, and a <code>while</code> loop keeps running while a condition is true.</p><pre><code class="python-code">for i in range(3):\n    print(i)</code></pre><p>This prints 0, 1, and 2.</p>`;
    }

    if (prompt.includes('list') || prompt.includes('dictionary') || prompt.includes('tuple') || prompt.includes('set')) {
      return `<h3>Python collection types</h3><p>Lists, tuples, sets, and dictionaries all store multiple values, but they behave differently. Lists are ordered and changeable; tuples are ordered and fixed; sets store unique values; dictionaries store key-value pairs.</p><pre><code class="python-code">grades = [90, 85, 78]\nstudent = {"name": "Ava", "score": 90}</code></pre><p>Use the right collection based on what you need to store and change.</p>`;
    }

    if (prompt.includes('function') || prompt.includes('def')) {
      return `<h3>Functions</h3><p>A function is a reusable block of code. It receives inputs, performs work, and often returns a result.</p><pre><code class="python-code">def add(a, b):\n    return a + b\n\nprint(add(3, 4))</code></pre><p>This prints 7.</p>`;
    }

    return `<h3>Python Teaching Answer</h3><p>I can help with that concept. A good way to approach it is to break the problem into inputs, logic, and output. Then test the code with a few simple examples.</p><p>For example, if you are learning a concept like loops, variables, or try/except, you can test it with a tiny script first, then expand it.</p><p>Ask me a specific Python question and I will explain it step by step.</p>`;
  }

  generateResponseInternal(userPrompt, userState) {
    const prompt = String(userPrompt || '').trim();
    if (!prompt) return '<p>Ask a Python question and I will explain it clearly.</p>';

    const questions = this.splitQuestions(prompt);
    if (questions.length > 1) {
      return this.buildMultiQuestionResponse(questions, userState);
    }

    return this.generateSingleAnswer(prompt, userState, false);
  }
